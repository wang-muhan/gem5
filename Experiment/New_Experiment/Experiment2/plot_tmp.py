...
path = "Experiment/New_Experiment/Experiment2/result2.txt"
...
path = "Experiment/New_Experiment/Experiment2/result2_tmp.txt"
with open(path, 'r') as file:
    lines = file.readlines()
    cnt = 0
    for synthetic in synthetics:
        for latency in [2,4]:
            for inj_rate in inj_rates:
                ...
                print(f"{synthetic}_TORUS_inj{inj_rate}_latency{latency}_numcpu{num_cpu}\n")
                # breakpoint()
                assert lines[10*cnt+2-1] == f"{synthetic}_TORUS_inj{inj_rate}_latency{latency}_numcpu{num_cpu}\n"
                reception_rate[2][(synthetic,latency,inj_rate)] = extract_reception_rate(lines[10*cnt+10-1])
                print(reception_rate[2][(synthetic,latency,inj_rate)])
                cnt+=1
